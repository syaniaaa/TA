        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white light:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-black dark:text-black">
                    ...
                    {{-- search --}}
                    <form action="{{ route('fuzzy_set.search') }}" method="GET" class="py-3 px-4 flex justify-end">
                    ...
                    <div class="mt-4">
                        @if ($fuzzy_sets->isNotEmpty())
                            @foreach ($fuzzy_sets as $fuzzy_set)
                            @endforeach
                        @else
                            <p>fuzzy_set Tidak Ada</p>
                        @endif
                    </div>
                    {{-- end search --}}

                    <div class="flex space-x-4">
                        <x-primary-button tag="a" href="{{ route('fuzzy_set.create') }}"><svg
                            ...
                            <tr>
                                ...
                                <td class="flex space-x-2">
                                    ...
                                    <x-danger-button x-data=""
                                        x-on:click.prevent="$dispatch('open-modal', 'confirm-fuzzy_set-deletion')"
                                        x-on:click="$dispatch('set-action', '{{ route('fuzzy_set.destroy', $fuzzy_set->id) }}')"><svg
                                            class="w-4 h-4 text-gray-100 dark:text-white" aria-hidden="true"
                                            xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            fill="none" viewBox="0 0 24 24">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <x-modal name="confirm-fuzzy_set-deletion" focusable maxWidth="xl">
